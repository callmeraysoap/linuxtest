...
int mqtt_Subscribe_topic(unsigned char *buf, unsigned int  identifier,char * topic, unsigned char qos) {
    int i = 0;
    int topic_len = strlen(topic);
    int Datalen;
    /*
    ...
    */
    ...
    /* 报文标识符 控制报文必须包含一个非零的 16 位报文标识符（Packet Identifier）*/
    buf[i++] = (unsigned char)(identifier >> 8); 
    buf[i++] = (unsigned char)identifier;
    /*主题长度*/
    buf[i++] = (unsigned char)(topic_len >> 8); 
...
int mqtt_keeplive(unsigned char *buf) {
    int i = 0;
...
int main() {
    ...
    // // 4. 发送 CONNECT 报文
    int len = mqtt_connect_packet(buffer, "testClient2");
    ...
    uint16_t idf_cnt = 1;
    len = mqtt_Subscribe_topic(buffer, idf_cnt,"a/b", 0);
    write(sock, buffer, len);
    idf_cnt++;
    while(1) {
